...
public class EmployeeController {
	...
	public ModelAndView searchEmployee(ModelAndView model, @ModelAttribute Employee searchEmployee) throws IOException {
        List<Employee> listEmployee =  employeeService.getAllEmployees();
        listEmployee = listEmployee.stream().filter((var) -> ( searchEmployee.getSalary() ==0.0 || var.getSalary() >= searchEmployee.getSalary())
        		&& ( searchEmployee.getBand() == '0' || var.getBand() == searchEmployee.getBand())
        		&& var.getEmpName().startsWith(searchEmployee.getEmpName())).collect(Collectors.toList());
        model.addObject("listEmployee", listEmployee);
        model.addObject("searchEmployee", new Employee());
		model.setViewName("home");
		return model;
