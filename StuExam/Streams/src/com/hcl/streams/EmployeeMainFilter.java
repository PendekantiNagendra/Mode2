...
public class EmployeeMainFilter {
	public static void main(String[] args) {
		...
		list.stream().filter((emp) -> emp.getBand() == 'M').forEach((arg) -> {
			System.out.println(arg.getEmpName());
		});
		list.stream().filter((emp) -> emp.getEmpNo() > 20).forEach((arg) -> {
			System.out.println(arg.getEmpNo());
		});
	}
